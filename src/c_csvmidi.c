...
static void outbyte(const byte c)
{
	...
	if (trackbuf == NULL) {
	  Rprintf("%s: Unable to allocate memory to expand track buffer to %d bytes.\n", PROG, trackbufl);
    	}
...
static int getCSVline(FILE *fp)
{
	...
	s = (char *) malloc(sl);
	if (s == NULL) {
	  Rprintf("%s: Unable to allocate %d byte CSV input line buffer.\n",
           PROG, sl);
	}
    }
	...
	if (ll >= (sl - 1)) {
	    ...
	    s = (char *) realloc(s, sl);
	    if (s == NULL) {
	      Rprintf("%s: Unable to expand CSV input line buffer to %d bytes.\n",
               PROG, sl);
	      /*	exit(2); */
	    }
	}
...
static void clamp(long *value, const long minval, const long maxval, const char *fieldname)
{
      ...
      Rprintf("%s out of range.  Value (%ld) outside limits of %ld to %ld",
...
void c_csvmidi(char **infile, char **outfile)
  {
    struct mhead mh;
    struct mtrack mt;
    FILE *fp , *fo ;
...
/*            Warn("Missing track number (field 1)"); */
      Rprintf("Missing track number (field 1)");
	    continue;
	}
	tl = track;
	...
	if (CSVscanField(&f, &flen)) {
	  ...
	  Rprintf("Missing absolute time (field 2)");
	    continue;
	}
	clamp(&abstime, 0, 0x7FFFFFFF, "Field 2 (Time)");

	if (!CSVscanField(&f, &flen)) {
    Rprintf("Missing absolute time (field 3)");
	  continue;
	}
	...
	if (rtype < 0) {
  Rprintf("Unknown record type: \"%s\"", f);
continue;
	}
	...
	switch (rtype) {
	    ...
	    case Marker(Header):
		if (headerseen) {
		  Rprintf("Duplicate header record");
		    continue;
		}
		Nfields(3);
		...
		if (intrack != 0) {
		Rprintf("Previous track end missing");
		    continue;
		}
		intrack = track;
		tabstime = 0;
		...
		if (intrack != 0) {
               ...
               Rprintf("Last track end missing");
		}
		continue;
	}
	...
	if (track != intrack) {
	    if (intrack == 0) {
	      Rprintf("Event not within track");
		continue;
	    } else {
	      Rprintf("Incorrect track number in event");
		continue;
	    }
	}
	...
	if (abstime < tabstime) {
	  Rprintf("Events out of order; this event is before the previous.");
	    continue;
	}
...
#define ClampChannel	Clamp(4, 0, 15, "Channel")
    ...
    Rprintf("Field 5 has invalid major/minor indicator \"%s\"", f);
    	    	  of[1] = 0;
		}
		outbytes(of, 2);
		break;
    ...
    if (!eofseen) {
      Rprintf("%s: Missing End_of_file record.\n", PROG);
    }
}
