/*
   ...
   Modified to be called from within R in January 2024 by Erich Neuwirth
...
fprintf(fo, " (Len=%ld  Trk=%02x) ", len, *trk);
		    ...
		    default:
			if (verbose) {
            Rprintf("Unknown meta event type");
			}
	   ...
	   default:
	      if (verbose) {
	        Rprintf("Unknown event type 0x%02X.\n", evt);
	      }
...
void c_midicsv(char **infile, char **outfile)
{
    struct mhead mh;
    FILE *fp, *fo;
    long track1;
...
*/
    ...
    if (memcmp(mh.chunktype, "MThd", sizeof mh.chunktype) != 0) {
      Rprintf("input file is not a Standard MIDI File.\n");
    }
    if (verbose) {
  Rprintf("Format %d MIDI file.  %d tracks, %d ticks per quarter note.\n",
	...
	readMidiTrackHeader(fp, &mt);
        if (memcmp(mt.chunktype, "MTrk", sizeof mt.chunktype) != 0) {
          Rprintf("Track %d header is invalid.\n", i + 1);
	}

	if (verbose) {
	  Rprintf("Track %d: length %ld.\n", i + 1, mt.length);
	}
        fprintf(fo, "%d, 0, Start_track\n", i + 1);

	trk = (byte *) malloc(mt.length);
	if (trk == NULL) {
	  Rprintf("%s: Cannot allocate %ld bytes for track.",
           progname, mt.length);
	}
	track1l=0;

	size_t res = fread((char *) trk, (int) mt.length, 1, fp);
	dummy = res;
	res = dummy;
...
static const R_CMethodDef CEntries[] = {
  {"c_midicsv", (DL_FUNC) &c_midicsv, 2},
  {"c_csvmidi", (DL_FUNC) &c_csvmidi, 2},
  {NULL, NULL, 0}
};

#include <Rversion.h>
void R_init_midicsvR(DllInfo *dll)
{
